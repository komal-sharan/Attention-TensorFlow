...
class Answer_Generator():
    ...
    def build_model(self):
        ...
        acc, acc_op = tf.metrics.accuracy(labels=label, predictions=tf.argmax(scores_emb,1))
...
# Data input setting
input_img_h5 = '/home/ksharan1/visualization/san-vqa-tensorflow/data_img_pool5_fresh.h5'
...
# Check point
checkpoint_path = 'vqahat_model/san_lstm_att_accuracies'
...
def kl_divergence(p, q):
    newploss=(p+0.00001)
    newqloss=(q+0.00001)
    return tf.reduce_sum(((p * tf.log(newploss/newqloss))))
...
def train():
    ...
    #dataset_val, img_feature_val, train_data_val = get_data_val()
    #this is the one used for training
    newp=open('save4.pkl')# Python 3: open(..., 'wb')
    datanew=pickle.load(newp)
    num_train = datanew['question'].shape[0]

    #this is used for validation
    newpval=open('save4test.pkl')
    datanewval=pickle.load(newpval)
    num_val=datanewval['question'].shape[0]
    ...
    ########################### checking the gpu options to allow for usage ################
    ...
    #print "train_op"
    #print train_op
    #train_op= tf.zeros([3, 4], tf.int32)
    ...
    for itr in range(max_itr):
        ...
        current_question = datanew['question'][index,:]
        current_length_q = datanew['length_q'][index]
        current_answers = datanew['answers'][index]
        current_img_list = datanew['img_list'][index]
        current_img_vqa = datanew['vqahat'][index,:]
        ...
        _, loss,summary = sess.run(
                [train_op, tf_loss,model.allsumaries],
                feed_dict={
        ...
        if np.mod(itr, 100) == 0:
            ...
            current_val_question = datanewval['question'][index_val,:]
            current_val_length_q = datanewval['length_q'][index_val]
            current_val_answers = datanewval['answers'][index_val]
            current_val_img_list = datanewval['img_list'][index_val]
            current_val_img_vqa = datanewval['vqahat'][index_val,:]
            current_val_img = img_feature[current_img_list,:]
            ...
            writer.add_summary(valsummary,itr )
        ...
        if np.mod(itr, 5000) == 0:
            print "Iteration ", itr, " is done. Saving the model and validating ..."

            saver.save(sess, os.path.join(checkpoint_path, 'model'), global_step=itr)
            last_model= os.path.join(checkpoint_path, 'model')+str(itr)
            print 'loading  100 images for calculating accuracy from dataset...'
