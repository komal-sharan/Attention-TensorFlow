...
class Answer_Generator():
    ...
    def build_model(self):
        ...
        #self.cross_entropy_summary =tf.summary.scalar(["cross_entropy_loss"],cross_entropy)
        acc, acc_op = tf.metrics.accuracy(labels=label, predictions=tf.argmax(scores_emb,1))
        print "start"
        print vqa_image_prob
        print prob_att2
        self. prob_att2=prob_att2
        self. vqa_image_prob=vqa_image_prob
        KL = kl_divergence(prob_att2,vqa_image_prob)


        final_loss=  cross_entropy+KL
        onlycross=  tf.reduce_mean(cross_entropy)
        self.onlycross=onlycross
...
# Data input setting
input_img_h5 = '/home/ksharan1/visualization/san-vqa-tensorflow/data_img_pool5_v2_new.h5'
...
# Check point
checkpoint_path = 'vqahat_model/san_lstm_att_multitask'
checkpoint_path_for_restore='model/san_lstm_att3'
...
def kl_divergence(p, q):
    ...
    return tf.reduce_sum(((p * tf.log(newploss/newqloss))),1)

def kl_intermediate(p, q):
    newploss=(p+0.00001)
    newqloss=(q+0.00001)

    return  tf.log(newploss/newqloss),newploss,newqloss
...
def train():
    ...
    num_val = test_data['question'].shape[0]
    #this is for vqahat
        ...
        if alter:
            ...
            _,loss,onlycross,summary = sess.run(
                        ...
                        feed_dict={
                            ...
                            tf_question: current_vqahat_question,
